...
async def update_items_in_cart(cartsService: CartsService, ordersService: OrdersService, product_id: int, updateReq: UpdateItemsInCartReq, logged_in_user: LoggedInUser):
    if logged_in_user is None:
        raise HTTPException(404, "user not found")
    order = ordersService.get_order_by_id(logged_in_user.Id, "cart-state")
    if order is None:
